...
Disclaimer: The respective trademarks mentioned in the offering are owned by the respective companies. We do not provide a commercial license for any of these products. This listing has an open-source license. MongoDB(R) is run and maintained by MongoDB, which is a completely separate project from Drycc.
...
This chart bootstraps a [MongoDB(&reg;)](https://github.com/drycc/containers/tree/main/drycc/mongodb) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
Refer to the [chart documentation for more information on each of these architectures](https://docs.drycc.com/kubernetes/infrastructure/mongodb/get-started/understand-architecture/).
...
| `image.repository`       | MongoDB(&reg;) image registry                                                                                                                                | `drycc/mongodb`      |
...
| `tls.image.repository`   | Init container TLS certs setup image repository                                                                                                              | `drycc/nginx`        |
...
| `externalAccess.autoDiscovery.image.repository`          | Init container auto-discovery image repository                                                                                                  | `drycc/kubectl`     |
...
| `persistence.mountPath`                       | Path to mount the volume at                                                                                                           | `/drycc/mongodb`  |
...
| `hidden.persistence.mountPath`                       | The path the volume will be mounted at, useful when using different MongoDB(&reg;) images.           | `/drycc/mongodb`  |
...
| `metrics.image.repository`                   | MongoDB(&reg;) Prometheus exporter image repository                                                                   | `drycc/mongodb-exporter` |
...
The [Drycc MongoDB(&reg;) image](https://github.com/drycc/containers/tree/main/drycc/mongodb) supports the use of custom scripts to initialize a fresh instance. In order to execute the scripts, two options are available:
...
Refer to the [chart documentation for more details and configuration examples](https://docs.drycc.com/kubernetes/infrastructure/mongodb/configuration/configure-external-access-replicaset/).
...
Refer to the chart documentation for more information on, and examples of, configuring and using [sidecars and init containers](https://docs.drycc.com/kubernetes/infrastructure/mongodb/configuration/configure-sidecar-init-containers/).

## Persistence

The [Drycc MongoDB(&reg;)](https://github.com/drycc/containers/tree/main/drycc/mongodb) image stores the MongoDB(&reg;) data and configurations at the `/drycc/mongodb` path of the container.
...
Refer to the [chart documentation for an example of a custom rule](https://docs.drycc.com/kubernetes/infrastructure/mongodb/administration/use-prometheus-rules/).
...
Refer to the [chart documentation for more information on enabling TLS](https://docs.drycc.com/kubernetes/infrastructure/mongodb/administration/enable-tls/).
...
[Learn more about this change and related upgrade considerations](https://docs.drycc.com/kubernetes/infrastructure/mongodb/administration/upgrade-helm3/).
...
- This version introduces `drycc/common`, a [library chart](https://helm.sh/docs/topics/library_charts/#helm) as a dependency. More documentation about this new utility could be found [here](https://github.com/drycc/charts/tree/master/drycc/common#drycc-common-library-chart). Please, make sure that you have updated the chart dependencies before executing any upgrade.
...
You can find more information in the [`drycc/mongodb` image README](https://github.com/drycc/containers/tree/main/drycc/mongodb#readme).
