...
- name: prepare-config
  ...
  args:
    ...
    - |
      ...
      [[ -f "/drycc/kafka/data/meta.properties" ]] && ID="$(grep "node.id" /drycc/kafka/data/meta.properties | awk -F '=' '{print $2}')"
      ...
      if [[ "controller" =~ "$POD_ROLE" ]]; then
          ...
          for ((i = 0; i < {{ int .context.Values.controller.replicaCount }}; i++)); do
              ...
              [[ $i -eq $POD_ID ]] && [[ -f "/drycc/kafka/data/meta.properties" ]] && DIR_ID="$(grep "directory.id" /drycc/kafka/data/meta.properties | awk -F '=' '{print $2}')"
  ...
  volumeMounts:
    ...
    - name: kafka-sasl
      mountPath: /opt/drycc/kafka/config/secrets
      readOnly: true
    {{- end }}
{{- end -}}
