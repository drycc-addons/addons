...
| `controller.logPersistence.mountPath`                          | Mount path of the Kafka logs volume                                                                                                                                                           | `/opt/drycc/kafka/logs` |
...
| `broker.logPersistence.mountPath`                          | Mount path of the Kafka logs volume                                                                                                                                                           | `/opt/drycc/kafka/logs` |
...
RUN mkdir -p /opt/drycc/kafka/plugins && \
    cd /opt/drycc/kafka/plugins && \
    curl --remote-name --location --silent https://search.maven.org/remotecontent?filepath=org/mongodb/kafka/mongo-kafka-connect/1.2.0/mongo-kafka-connect-1.2.0-all.jar
CMD /opt/drycc/kafka/bin/connect-standalone.sh /opt/drycc/kafka/config/connect-standalone.properties /opt/drycc/kafka/config/mongo.properties
```
...
If you are setting the `config` or `log4j` parameter, backwards compatibility is not guaranteed, because the `KAFKA_MOUNTED_CONFDIR` has moved from `/opt/drycc/kafka/conf` to `/bitnami/kafka/config`. In order to continue using these parameters, you must also upgrade your image to `docker.io/bitnami/kafka:2.4.1-debian-10-r38` or later.
