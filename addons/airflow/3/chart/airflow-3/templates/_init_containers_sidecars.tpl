...
- name: prepare-config
  ...
  args:
    ...
    - |
      ...
      cp /opt/drycc/airflow/airflow.cfg /emptydir/app-base-dir/airflow.cfg
    ...
    {{- end }}
      ...
      if [[ -f "/opt/drycc/airflow/config/airflow_local_settings.py" ]]; then
          cp /opt/drycc/airflow/config/airflow_local_settings.py /emptydir/app-conf-dir/airflow_local_settings.py
      else
  ...
  env:
    ...
    - name: AIRFLOW_DATABASE_SQL_CONN_FILE
      value: {{ printf "/opt/drycc/airflow/secrets/%s" (include "airflow.database.secretKey" .) }}
    {{- else }}
    ...
    {{- end }}
    {{- if or (eq .Values.executor "CeleryExecutor") (eq .Values.executor "CeleryKubernetesExecutor") }}
    {{- include "airflow.configure.redis" . | nindent 4 }}
    {{- end }}
...
- name: prepare-web-config
  ...
  args:
    ...
    - |
      ...
      # Copy the configuration files to the writable directory
      cp /opt/drycc/airflow/webserver_config.py /emptydir/app-base-dir/webserver_config.py
  ...
  env:
    ...
    - name: AIRFLOW_LDAP_BIND_PASSWORD_FILE
      value: "/opt/drycc/airflow/secrets/bind-password"
    {{- else }}
...
- image: {{ include "airflow.image" . }}
  ...
  volumeMounts:
    ...
    - name: empty-dir
      ...
      subPath: etc-ssh-dir
    - name: empty-dir
      mountPath: /opt/drycc/airflow/.ssh
      subPath: ssh-dir
...
{{ include "airflow.defaultInitContainers.shared" . }}
    ...
    - name: dags-ssh-key
      mountPath: /opt/drycc/airflow/.ssh/dags-ssh-key
  ...
  args:
    ...
    {{- if or .Values.dags.sshKey .Values.dags.existingSshKeySecret }}
      export GIT_SSH_COMMAND="ssh -i /opt/drycc/airflow/.ssh/dags-ssh-key -o StrictHostKeyChecking=no"
    {{- end }}
...
{{ include "airflow.defaultInitContainers.shared" . }}
    ...
    - name: plugins-ssh-key
      mountPath: /opt/drycc/airflow/.ssh/plugins-ssh-key
  ...
  args:
    ...
    {{- if or .Values.plugins.sshKey .Values.plugins.existingSshKeySecret }}
      export GIT_SSH_COMMAND="ssh -i /opt/drycc/airflow/.ssh/plugins-ssh-key -o StrictHostKeyChecking=no"
    {{- end }}
...
{{- range .Values.dags.repositories }}
- name: empty-dir
  mountPath: /opt/drycc/airflow/dags/git_{{ include "airflow.dagsPlugins.repository.name" . }}
  {{- if .path }}
...
{{- range .Values.plugins.repositories }}
- name: empty-dir
  mountPath: /opt/drycc/airflow/plugins/git_{{ include "airflow.dagsPlugins.repository.name" . }}
  {{- if .path }}
